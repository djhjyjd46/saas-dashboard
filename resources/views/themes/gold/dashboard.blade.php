        ...
        <div class="pr-8 flex flex-col gap-3">
            ...
            <div class="flex gap-3 items-center">
                <div class="relative w-full"
                    x-data="{ showTip: false, tipText: '' }"
                    x-on:sync-cooldown.window="tipText = $event.detail.message; showTip = true; setTimeout(() => showTip = false, 2000)">
                    ...
                    <div x-show="showTip" x-transition
                        class="absolute left-1/2 -translate-x-1/2 -top-10 px-3 py-2 rounded-lg bg-[#111317] border border-yellow-500/30 text-yellow-400 text-xs whitespace-nowrap z-50">
    ...
    <div class="right w-4/5 bg-[#434141] rounded-2xl px-5 py-7 flex flex-col gap-8 overflow-y-auto">
        <div class="cards grid grid-cols-4 gap-5">
            <div class="card bg-card rounded-2xl px-5 py-2 h-[140px]">
                ...
                <div class="grafik h-8">
                    <svg class="w-full h-full" viewBox="0 0 100 20" preserveAspectRatio="none">
                        <polyline fill="none" stroke="#F1D38C" stroke-width="2" points="{{ $spendSparkline }}">
                        </polyline>
                    </svg>
                </div>
            </div>
            <div class="card bg-card rounded-2xl px-5 py-2 h-[140px]">
                <div class="flex justify-between mb-4">
                    <div class="top flex justify-between w-full items-center">
                        <span class="gradient-text">ЗАЯВКИ (ДИРЕКТ)</span>
                        <div class="w-8 h-8 rounded-lg bg-gold/10 flex items-center justify-center">
                            <svg class="w-4 h-4 text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="card bg-card rounded-2xl px-5 py-2 h-[140px]">
                <div class="flex justify-between mb-4">
                    <div class="top flex justify-between w-full items-center">
                        <span class="gradient-text">ЛИДЫ (CRM)</span>
                        <div class="w-8 h-8 rounded-lg bg-gold/10 flex items-center justify-center">
                            <svg class="w-4 h-4 text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="grafik h-8">
                    <svg class="w-full h-full" viewBox="0 0 100 20" preserveAspectRatio="none">
                        <polyline fill="none" stroke="#F1D38C" stroke-width="2" points="{{ $leadsSparkline }}">
                        </polyline>
                    </svg>
                </div>
            </div>
            <div class="card bg-card rounded-2xl px-5 py-2 h-[140px]">
                <div class="flex justify-between mb-4">
                    <div class="top flex justify-between w-full items-center">
                        <span class="gradient-text">КВАЛ. ЛИДЫ (CRM)</span>
                        <div class="w-8 h-8 rounded-lg bg-gold/10 flex items-center justify-center">
                            <svg class="w-4 h-4 text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="bg-card rounded-2xl border border-white/5">
            <table class="gold-dashboard-table overflow-y-auto">
                <thead>
                    <!-- Row 1: Groups -->
                    <tr>
                        <th rowspan="2" wire:click="sortTable('name')" style="cursor:pointer;">Направление @if($tableSortBy === 'name') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th rowspan="2" wire:click="sortTable('spend')" style="cursor:pointer;">Расход @if($tableSortBy === 'spend') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th colspan="2">Лиды (CRM)</th>
                        <th colspan="3">Квал. Лиды (CRM)</th>
                        <th colspan="4">Продажи</th>
                        <th rowspan="2" wire:click="sortTable('revenue')" style="cursor:pointer;">Заявки (Директ) @if($tableSortBy === 'revenue') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                    </tr>
                    <!-- Row 2: Sub-columns -->
                    <tr>
                        <th wire:click="sortTable('cpl')" style="cursor:pointer;">CPL @if($tableSortBy === 'cpl') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('leads')" style="cursor:pointer;">Кол-во @if($tableSortBy === 'leads') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('cr1')" style="cursor:pointer;">CR1 @if($tableSortBy === 'cr1') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('cpl2')" style="cursor:pointer;">CPL2 @if($tableSortBy === 'cpl2') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('qual_leads')" style="cursor:pointer;">Кол-во @if($tableSortBy === 'qual_leads') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('cr2')" style="cursor:pointer;">CR2 @if($tableSortBy === 'cr2') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('cr3')" style="cursor:pointer;">CR3 @if($tableSortBy === 'cr3') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('cps')" style="cursor:pointer;">CPS @if($tableSortBy === 'cps') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                        <th wire:click="sortTable('deals')" style="cursor:pointer;">Кол-во @if($tableSortBy === 'deals') {{ $tableSortDir === 'asc' ? '↑' : '↓' }} @endif</th>
                    </tr>
                </thead>
                <tbody>
                    <!-- Total Row -->
                        ...
                        @if (in_array($group['id'], $expandedCategories))
                            @foreach ($group['campaigns'] as $camp)
                                ...
                                <tr wire:click="openCampaign({{ $camp['id'] }})" style="cursor:pointer;">
                                    <td style="padding-left: 60px; font-weight: 400; width: 330px;">
                                        {{ $camp['name'] }}</td>
                ...
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <span class="text-white/40 text-xs">id:{{ $currentCampaign?->external_id ?? '—' }}</span>
                        </div>
                    </div>
